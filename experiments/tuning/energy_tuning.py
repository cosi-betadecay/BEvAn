...
from physics.ground_truths import ground_truth_annihilation, ground_truth_bdecay, ground_truth_compton
...
def annihilation_extractor_energy(
    cfg: omegaconf.dictconfig.DictConfig,
    ref_energy: int = 511,
) -> None:
    ...
    for event in tqdm(
        ...
        score_energy = detected_511_event_energy(ref_energy, event, cfg)
        ...
        scores_bdecay.append(score_energy)
...
def main(
    ...
    scores_bdecay, ground_truths_bdecay, ground_truths_annihilation, ground_truths_compton = (
        annihilation_extractor_energy(cfg)
    )

    # Beta decay
    true_events_energy_bdecay = []
    false_events_energy_bdecay = []
    # Annihilations
    true_events_energy_annihilation = []
    false_events_energy_annihilation = []
    # Compton
    true_events_energy_compton = []
    false_events_energy_compton = []

    # Mapping runs
    for i in tqdm(range(len(ground_truths_bdecay)), desc="Creating results", unit=" events"):
        if ground_truths_bdecay[i]:
            true_events_energy_bdecay.append(scores_bdecay[i].item())
        else:
            false_events_energy_bdecay.append(scores_bdecay[i].item())
        if ground_truths_annihilation[i]:
            true_events_energy_annihilation.append(scores_bdecay[i].item())
        else:
            false_events_energy_annihilation.append(scores_bdecay[i].item())
        if ground_truths_compton[i]:
            true_events_energy_compton.append(scores_bdecay[i].item())
        else:
            false_events_energy_compton.append(scores_bdecay[i].item())

    # Creating runs
    runs = [
        ("energy_bdecay", scores_bdecay.cpu().numpy()),
        ("energy_true_events_bdecay", true_events_energy_bdecay),
        ("energy_false_events_bdecay", false_events_energy_bdecay),
        ("energy_annihilation", scores_bdecay.cpu().numpy()),
        ("energy_true_events_annihilation", true_events_energy_annihilation),
        ("energy_false_events_annihilation", false_events_energy_annihilation),
        ("energy_compton", scores_bdecay.cpu().numpy()),
        ("energy_true_events_compton", true_events_energy_compton),
        ("energy_false_events_compton", false_events_energy_compton),
    ]
    ...
    for label, data in runs:
        ...
        wandb.init(project="cosi-betadecay-annihilation", name=label, reinit=True)

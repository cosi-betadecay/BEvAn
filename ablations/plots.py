...
# The deployed label window (resolution sigmas); marked on the sweep so the reference
...
def plot_label_window(
    ...
    across the non-``exclude`` datasets with an amber ±std band (geometry-to-geometry
    spread), teal error bars on the markers give the mean per-dataset seed spread
    (split noise, from the ``<metric>_std`` record keys of a multi-seed run; absent
    on single-seed records), and a dashed vertical marks the deployed window
    (:data:`DEPLOYED_N_STD`). Roughly flat curves in the same band as the marked
    point are the robustness claim: the study does not hinge on the cut.
    ...
    for ax, (key, label) in zip(axes.flat, COMPARISON_METRICS, strict=True):
        means, stds, seed_stds = [], [], []
        for n_std in n_std_values:
            records = [per_n[n_std] for per_n in included.values() if n_std in per_n]
            mean, std = mean_std([r.get(key, float("nan")) for r in records])
            means.append(mean)
            stds.append(std)
            seed_stds.append(mean_std([r.get(f"{key}_std", float("nan")) for r in records])[0])
        means_arr, stds_arr = np.array(means), np.array(stds)
        seed_arr = np.nan_to_num(np.array(seed_stds), nan=0.0)
        ax.fill_between(
            x,
            means_arr - stds_arr,
            means_arr + stds_arr,
    ...
    fig.tight_layout()
    return save(fig, Path(save_dir) / "label_window.png")

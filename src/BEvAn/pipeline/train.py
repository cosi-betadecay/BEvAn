...
# The hardcoded per-bucket bin budget
REFERENCE_BINS: dict[int, int] = {1: 25, 2: 8, 3: 35}


def flatten_config(config: dict) -> dict[str, float]:
    """Flatten a config dict to scalar columns for logging / CSV output."""
    out = {f"n_bins_{b}": float(v) for b, v in sorted(config["n_bins"].items())}
...
class Trainer:
    ...
    # Per-bucket fitted models; absent until fit() or load() (guarded by hasattr).
    ...
    def __init__(
        ...
        """Configure the per-bucket bin budget and Laplace smoothing strengths.

        Args:
            n_bins: Bins-per-axis for each hit-multiplicity bucket's density
                terms; defaults to the reference budget :data:`REFERENCE_BINS`.
        ...
        """
        self.n_bins = dict(n_bins) if n_bins is not None else dict(REFERENCE_BINS)
    ...
    def config(self) -> dict[str, dict[int, int] | float]:
        ...
        Returns:
            ``{"n_bins": ..., "joint_smoothing": ..., "marginal_smoothing": ...}``.
        """
        return {
            "n_bins": self.n_bins,
    ...
    def save(
        ...
        load, and free-form provenance. Every value is a plain scalar, string, or
        tensor, so the artifact reloads without re-fitting and without a live
        MEGAlib reader.
    ...
    def fit_bucket(
        self, bd: dict[str, torch.Tensor], bg: dict[str, torch.Tensor], bucket: int
        ...
        model: dict[str, int | list[dict]] = {"n_beta": n_beta, "n_bg": n_bg, "terms": []}
        ...
        if bucket == 1:
            ...
            self.add_2d(model, bd, bg, "delta_E", "arm", "linear", None, n_bins)
        else:  # bucket 3
            self.add_2d(model, bd, bg, "delta_E", "arm", "linear", None, n_bins)
    ...
    def add_1d(
        ...
        Per-class densities share the pooled-data edges and use the marginal
        smoothing. No-op unless both classes have a finite value (a ratio needs
        both densities).
        ...
        Args:
            ...
            n_bins: Bins-per-axis for this bucket.
        """
    ...
    def add_2d(
        ...
        Per-class densities share the pooled-data edges and use the joint
        smoothing. No-op unless both classes have a finite pair (a ratio needs
        both densities).

...
def theta_kin(energies: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        Scatter angle in radians, shape ``(B,)`` (a 0-dim scalar for ``(n,)`` input).
    """
...
def per_subset_back_to_back(
    ...
    for v in range(n):
        ...
        push = (dE2.abs() - 2.0 * tolerance).clamp(min=0.0) / 511.0
...
def arm_fixed_size(
    ...
    away from 0 in the direction of the energy miss and in proportion to it. The
...
def arm(
    ...
    residual), and returns the signed feature of whichever of

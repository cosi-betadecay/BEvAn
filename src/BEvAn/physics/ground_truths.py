...
def calculate_tolerance(n_std: int = 5) -> float:
...
def ground_truth_bdecay(event: MSimEvent, n_std: int = 5) -> bool:
...
def bdecay_label_score(event: MSimEvent) -> float:
    ...
    the ``label_window`` sweep relabel for every window from a single extraction instead

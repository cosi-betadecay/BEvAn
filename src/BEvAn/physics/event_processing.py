...
# Complexity guard: events above MAX_HITS hits are truncated to their
# highest-energy hits first, bounding the per-event subset count.
MAX_HITS = 25
...
def candidate_perms(r: int) -> torch.Tensor:
    ...
    unchanged. Cached per ``r`` (the result is a pure function of it); callers
...
@functools.cache
def hit_subsets(n_hits: int, r: int) -> torch.Tensor:
    """All size-``r`` ascending hit-index subsets of ``n_hits`` hits.

    Cached per ``(n_hits, r)`` (bounded by :data:`MAX_HITS`); callers must not
    ...
    """
    return torch.tensor(list(itertools.combinations(range(n_hits), r)), dtype=torch.long)
...
def ckd_orderings(
    ...
    ``ckd_order_max`` (NaN/no-scatter counts as consistent). Subsets with no
...
def event_data_processing(
    ...
    Enumerates hit subsets up to size 8, orders each by
    Compton-kinematic-discrepancy consistency, and pads them to a fixed width
    with a NaN sentinel row so downstream gathers stay rectangular. Events above
    :data:`MAX_HITS` hits are truncated to their highest-energy hits first,
    bounding the per-event subset count (see the constant's rationale).
    ...
    # gathered combo tensors, which is the only GPU-shaped work here. One GetHTAt
    # fetch per hit — each accessor is a PyROOT round-trip.
    hit_energies: list[float] = []
    hit_positions: list[tuple[float, float, float]] = []
    for i in range(n_hits):
        ht = event_sim.GetHTAt(i)
        pos = ht.GetPosition()
        hit_energies.append(ht.GetEnergy())
        hit_positions.append((pos.X(), pos.Y(), pos.Z()))
    energies = torch.tensor(hit_energies, dtype=torch.float32, device="cpu")
    positions = torch.tensor(hit_positions, dtype=torch.float32, device="cpu")

    # Complexity guard (see MAX_HITS): keep the highest-energy hits via a STABLE
    # descending argsort (deterministic on energy ties), then re-sort the kept
    # indices ascending so the surviving hits preserve their original order and
    # everything downstream is unchanged.
    if n_hits > MAX_HITS:
        keep = torch.argsort(energies, descending=True, stable=True)[:MAX_HITS].sort().values
        energies = energies[keep]
        positions = positions[keep]
        n_hits = MAX_HITS

    # Build all hit combinations, vectorized per size group.

...
def metrics(counts: dict[str, int]) -> dict[str, float]:
    ...
    :func:`modeling.calculate_probabilities.log_confusion` reuses it
...
def best_f1_threshold(scores: torch.Tensor, labels: torch.Tensor) -> tuple[float, float]:
    ...
    Sweeps every candidate threshold and returns the winning cut alongside its
    F1, so a decision rule can be deployed at it. Cuts fall only between distinct

...
def test_run_fits_reference_config_on_energy_features(
    tiny_ds: harness.DatasetPaths, tiny_cache: Path
) -> None:
    """Verify the ablation fits the reference config on energy-ordered features over all events."""
...
def test_run_deploys_the_dedicated_prior_not_the_training_counts(
    tiny_ds: harness.DatasetPaths, tiny_cache: Path
) -> None:
...
def test_run_writes_plot_set_when_asked(
    tiny_ds: harness.DatasetPaths, tiny_cache: Path, tmp_path: Path
) -> None:
    """Verify the ablation writes its confusion and ROC plots when a plots dir is given."""
    train_energy, eval_energy = harness.extract_split(tiny_ds, ordering="energy")
    out = tmp_path / "no_ckd"
    counts = harness.dedicated_prior_counts(tiny_ds)

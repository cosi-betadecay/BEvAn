...
def test_run_reports_every_stage(tiny_ds, tiny_cache):
...
def test_run_bypasses_the_feature_cache(tiny_ds, tiny_cache):
...
def test_n_events_counts_both_classes_and_every_bucket(tiny_split):
...
def test_density_cells_match_the_configured_bin_budget(tiny_split):
...
def test_artifact_kb_is_positive_and_matches_a_saved_file(tiny_split, tmp_path):
    train, _ = tiny_split
    trainer = harness.Trainer().fit(train)
...
def test_environment_names_the_machine():

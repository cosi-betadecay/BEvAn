...
def test_run_comparison_rejects_unknown_name(tiny_split):
...
def test_main_raises_without_matching_datasets(monkeypatch):
...
def test_main_end_to_end_writes_figures_and_tables(monkeypatch, tmp_path, tiny_ds, tiny_cache):
...
def test_main_multi_seed_writes_std_columns(monkeypatch, tmp_path, tiny_ds, tiny_cache):
    monkeypatch.setattr(harness, "discover_datasets", lambda: [tiny_ds])
    monkeypatch.setattr(main, "RESULTS_DIR", tmp_path / "results")
...
def test_main_respects_ablation_filter(monkeypatch, tmp_path, tiny_ds, tiny_cache):
    monkeypatch.setattr(harness, "discover_datasets", lambda: [tiny_ds])
    monkeypatch.setattr(main, "RESULTS_DIR", tmp_path / "results")
...
def test_parse_args_defaults_and_choices(monkeypatch, capsys):

...
def test_calculate_tolerance_is_n_sigma_of_fwhm():
    sigma = 2.25 / 2.355
...
def test_anni_process_energies_sums_secondary_hits():
    event = _event_with_anni([300.0, 211.0])
    assert anni_process_energies(event) == pytest.approx([511.0])


def test_anni_process_energies_one_entry_per_anni():
...
def test_anni_process_energies_counts_each_hit_once():
...
def test_anni_process_energies_empty_without_anni():
    event = SimEvent(1, hits=[SimHit((0.0, 0.0, 0.0), 511.0)], ias=[SimIA("COMP", 1, 0)])
    assert anni_process_energies(event) == []


def test_ground_truth_bdecay_thresholds_at_the_window():
...
def test_ground_truth_bdecay_window_scales_with_n_std():
...
def test_bdecay_label_score_matches_the_label():
...
def test_bdecay_label_score_inf_without_anni():

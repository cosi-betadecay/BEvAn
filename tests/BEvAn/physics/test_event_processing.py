...
def test_candidate_perms_counts_and_uniqueness():
...
def test_hit_subsets_are_ascending_combinations():
    s = hit_subsets(5, 2)
...
def test_energy_desc_order_stable_on_ties():
...
def test_orderings_for_size_dispatch():
...
def test_event_data_processing_empty_event():
    assert event_data_processing(SimEvent(1)) == (None, None, None)


def test_event_data_processing_shapes_and_padding():
...
def test_event_data_processing_size_2_orders_by_energy():
...
def test_event_data_processing_truncates_to_highest_energy_hits(monkeypatch):
...
def test_ckd_residual_nan_without_internal_scatter():
...
def test_ckd_residual_zero_for_consistent_sequence():

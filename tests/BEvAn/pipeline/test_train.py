...
def test_fit_builds_expected_terms_per_bucket(separable_data):
...
def test_fit_empty_class_yields_prior_only_model(separable_data):
    for f in FEATURES:
...
def test_save_load_roundtrip(tmp_path, separable_data):
...
def test_load_rejects_stale_artifacts(tmp_path, separable_data):
    trainer = Trainer().fit(separable_data)
    path = tmp_path / "model.pt"
    trainer.save(path)
    payload = torch.load(path, map_location="cpu", weights_only=False)

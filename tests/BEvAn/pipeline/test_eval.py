...
def test_evaluator_perfect_on_separable_data(
    separable_data: dict[str, dict[int, dict[str, torch.Tensor]]],
) -> None:
    """Classify perfectly separable data with zero errors."""
...
def test_evaluator_excludes_nonfinite_bucket_features(
    separable_data: dict[str, dict[int, dict[str, torch.Tensor]]],
) -> None:
    """Exclude events with non-finite bucket features rather than misclassifying them."""
    n = separable_data["bdecay"][1]["delta_E"].numel()
...
def test_classify_events_tags_match_labels(
    separable_data: dict[str, dict[int, dict[str, torch.Tensor]]],
) -> None:
    """Tag every event key with the class its features imply."""
    n = separable_data["bdecay"][1]["delta_E"].numel()
...
def test_classify_events_asserts_on_misaligned_keys(
    separable_data: dict[str, dict[int, dict[str, torch.Tensor]]],
) -> None:
    """Assert when key counts do not match the feature rows they index."""
    n = separable_data["bdecay"][1]["delta_E"].numel()
...
def test_pooled_log_ratio_and_prior_free_scores(
    separable_data: dict[str, dict[int, dict[str, torch.Tensor]]],
) -> None:
    """Derive perfect prior-free F1 and AUC from the pooled log-ratio scores."""
...
def test_prior_free_scores_none_gives_nan() -> None:
    """Return NaN scores when there are no pooled scores to rank."""
